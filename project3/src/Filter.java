...
public class Filter {
  ...
  DoublyLinkedList<Entry> ordered;
  ...
  public Filter(int size) {
    ...
    ordered = new DoublyLinkedList<Entry>();


  }

  public DoublyLinkedList<Entry> getOrdered() {
    return ordered;
  }

  public void setOrdered(DoublyLinkedList<Entry> ordered) {
...
//    Stdin Workaround
    ...
    filter.filt_linked_list();
//    Print result
    for (Entry e : filter.getOrdered()
      ) {
  ...
  public void filt_linked_list() {
    Collections.sort(this.entries); //sort along x for entries
    int index = 0;
    ListIterator<Entry> iterator = ordered.iterator();
    ...
    for (int i = 0; i < size; i++) {
      ...
      try {
        if (iterator.hasPrevious()) {
          while (Entry.Comparators.Y.compare(e, iterator.previous()) > 0) { // e's y is greater than previous'
            iterator.remove();
            if(!iterator.hasPrevious()){break;}

          }
          if (iterator.hasNext()) {
            iterator.next(); //go back
          }
